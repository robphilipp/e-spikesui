...
export function RasterChart(props: Props): JSX.Element {
    ...
    }), [props.margin, props.axisStyle, props.axisLabelFont, props.tooltip, props.tracker, props.svgStyle, width])
    ...
    console.log("RasterChart: before first useEffect")

    // when the series list changes, then clear out the data
    useEffect(
        () => {
            console.log("RasterChart useEffect for resetPlot; ", resetPlot)
            resetPlot();
        },
        [resetPlot]
    )
    ...
    const initializeAxes = useCallback(
         ...
         */
        (svg: SvgSelection, plotDimensions: PlotDimensions): Axes => {
    ...
    const onZoom = useCallback(
         ...
         * @param updatePlot Function to call to update the plot after calculation changes do to zoom
         */
        (transform: ZoomTransform, x: number, updatePlot: () => void) => {
    ...
    const tooltipX = useCallback(
         ...
         */
        (time: number, textWidth: number): number => {
            return Math.min(
                Math.max(
        ...
        [margin.left, tooltip.paddingLeft]
    )

    const tooltipY = useCallback(
        /**
         ...
         */
        (seriesName: string, textHeight: number): number => {
    ...
    const spikeLineHeight = useCallback(
        /**
         * @return The height of the spikes line
         */
        () => (plotDimRef.current.height - margin.top) / liveDataRef.current.size,
        [margin.top]
    )
    ...
    const handleShowTooltip = useCallback(
         ...
         */
        (datum: Datum, seriesName: string, spike: SVGLineElement) => {
        ...
        [spikesStyle.highlightColor, spikesStyle.highlightWidth, tooltipX, tooltipY]
    )
    ...
    const handleHideTooltip = useCallback(
        ...
        [spikesStyle.color, spikesStyle.lineWidth]
    )

    const magnifierLensAxisLabels = useCallback(
        /**
         ...
         */
        (ticks: Array<number>, selection: GSelection): Selection<SVGTextElement, number, SVGGElement, any> => selection
            .selectAll('text')
            .data(ticks)
            .enter()
            .append('text')
    ...
    const inMagnifier = useCallback(
         ...
         */
        (datum: Datum, x: number, xInterval: number): boolean => {
            const scale = axesRef.current?.xAxisGenerator.scale<ScaleLinear<number, number>>();
    ...
    const handleShowMagnify = useCallback(
        /**
        ...
        (svg: SvgSelection) => {
            ...
            if (containerRef.current && path) {
                ...
                const isMouseInPlot = mouseInPlotArea(x, y, width, height, margin);
        ...
        [height, width,
            inMagnifier, magnifier.axisOpacity, magnifier.magnification, magnifier.width, xFrom,
            margin,
            spikesStyle.lineWidth
        ]
    )

    const magnifierLens = useCallback(
        /**
         ...
         */
        (svg: SvgSelection, visible: boolean, height: number): BarMagnifierSelection | undefined => {
        ...
        [
            axisLabelFont.color, axisLabelFont.family, axisLabelFont.size, axisLabelFont.weight,
            axisStyle.color,
            handleShowMagnify, magnifier.color, magnifierLensAxisLabels,
            margin.top,
            tooltip.borderWidth
        ]
    )
    ...
    const addGridLines = useCallback(
        ...
        [margin.left, margin.top, plotGridLines.color]
    )

    const handleShowTracker = useCallback(
        /**
         * Callback when the mouse tracker is to be shown
         * @param path The d3 selection
         */
        (path: Selection<SVGLineElement, Datum, null, undefined> | undefined) => {
            if (containerRef.current && path) {
                const [x, y] = d3.mouse(containerRef.current);
                path
                    .attr('x1', x)
        ...
        [axisLabelFont.color, height, margin, tracker.color, width]
    )
    ...
    const trackerControl = useCallback(
         ...
         */
        (svg: SvgSelection, visible: boolean, height: number): TrackerSelection | undefined => {
        ...
        [
            axisLabelFont.color, axisLabelFont.family, axisLabelFont.size, axisLabelFont.weight,
            handleShowTracker, tracker.color, tracker.lineWidth,
            margin.top
        ]
    )
    ...
    const updatePlot = useCallback(
            ...
            if (containerRef.current && axesRef.current) {
                ...
                if (mainGRef.current === undefined) {
                    ...
                    const drag = d3.drag<SVGSVGElement, Datum>()
                        ...
                        .on("drag", () => onPan(d3.event.dx, updatePlot))
                    ...
                    const zoom = d3.zoom<SVGSVGElement, Datum>()
                        ...
                        .on("zoom", () => onZoom(d3.event.transform, d3.event.sourceEvent.offsetX - margin.left, updatePlot))
                    svg.call(zoom);
                } else {
        ...
        [
            ...
            onPan, onZoom,
            plotGridLines.visible,
            spikeLineHeight, spikesStyle.color, spikesStyle.lineWidth, spikesStyle.margin,
    ...
    const subscribe = useCallback(
         ...
         */
        (): Subscription => {
            console.log("RasterChart subscribe-callback; attempting to subscribe")
            resetPlot();
        ...
        [
            dropDataAfter, onUpdateData, onUpdateTime, resetPlot,
            seriesObservable, timeWindow, updatePlot, windowingTime
        ]
    )
    ...
    useEffect(
        () => {
            console.log("RasterChart: useEffect for subscription; shouldSubscribe", shouldSubscribe, "subscriptionRef.current", subscriptionRef.current, "subscribe", subscribe)
            if (shouldSubscribe && subscriptionRef.current === undefined) {
                console.log("RasterChart useEffect for subscription; attempting to subscribe")
                subscriptionRef.current = subscribe()

                // provide the subscription to the caller
                onSubscribe(subscriptionRef.current);
            } else if (!shouldSubscribe && subscriptionRef.current != undefined) {
                console.log("RasterChart useEffect for subscription; attempting to unsubscribe")
                subscriptionRef.current?.unsubscribe()
                subscriptionRef.current = undefined
            } else {
                console.log("RasterChart useEffect for subscription; unsubscribed and happy")
            }

            // stop the stream on dismount
            return () => {
                if (subscriptionRef.current !== undefined) {
                    console.log("RasterChart useEffect; unmounting; attempting to unsubscribe")
                    subscriptionRef.current.unsubscribe()
                    subscriptionRef.current = undefined
                } else {
                    console.log("RasterChart useEffect; unmounting; not subscribed")
                }
            }
        },
        [shouldSubscribe, subscribe, onSubscribe]
    )
